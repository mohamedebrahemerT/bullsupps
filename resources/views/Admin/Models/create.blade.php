...
@section('content')
         ...
         '<div class="div_inpute2">'+
         '</div>'+

             

         '<br> '+
         ' <a href="#"  style="background-color:orange;color: #fff" class="add_inputenew btn btn-info">add</i> </a>'+
         ...
         <script type="text/javascript">
          var x=1;
          $(document).on('click','.add_inputenew',function(){
 ...
 @push('js')
            <script type="text/javascript">
               ...
               $(document).on('click','.DELET_PRODUCT',function(){
                ...
                $.ajax({
              url:'{{ url('admin/DELET_PRODUCT/'.$Models->id ) }}',
              dataType:'json',
              type:'post',
              data:{
                _token:'{{ csrf_token()  }}'
          ...
          $('.DELET_PRODUCT1').html('<h1>'+data.message+'</h1>');
             ...
             $(document).on('click','#save_from_data1',function(){
              ...
              url:'{{ url('admin/Models/'.$Models->id) }}',
              dataType:'json',
              type:'post',
              data:From_data,
              beforeSend:function(){
          ...
          $('.sucess_message').html('<h1>'+data.message+'</h1>').removeClass('hidden');
              ...
              error:function(response){
               ...
               $(document).on('click','#approvedProdcut',function(){
 ...
 @push('js')
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      @if($Models->department_id) 
      

                    $.ajax({
       type:'GET',
       url:'{{ url('admin/dep_get_parent') }}',
       data:{country_id:'{{ $Models->department_id }}',select:'{{ $Models->city_id }}'},
       dataType: "html",
    success: function(data) {
        $('.city').html(data);
       $(".citycity").removeClass("hidden");
   ...
   $(".citycity").removeClass("hidden");
    ...
    error: function(data){
       ...
       type:'GET',
       url:'{{ url('admin/dep_get_parent') }}',
       data:{city_id:city_id,select:''},
       dataType: "html",
    success: function(data) {
        $('.price').html(data);
        $(".priceprice").removeClass("hidden");
  ...
  {!! Form::open(['url'=>url('admin/Models/'.$Models->id),'files'=>true,'method'=>'put','id'=>'save_from_data' ]) !!}

 
<!---------------------------------------------------------------------->
     <div class="form-group col-md-2 col-lg-2 col-sm-2"> 
                 {{ Form::label('country_id', trans('admin.chiosemainprog'))  }}
{{ Form::select('country_id',App\Department::where('parent',null)->pluck('dep_name_ar','id'),$Models->country_id,['class'=>'form-control country_id',"placeholder"=>"........"] )  }}
                     </div> 
...
<!----------------------------------------------------------------------->
            ...
            {!! Form::text('name_ar',$Models->name_ar,['class'=>'form-control']) !!}
            </div> 

            <div class="form-group col-md-2 col-lg-2 col-sm-2"> 
            {!! Form::label('name_en',trans('admin.name_en')) !!}
            {!! Form::text('name_en',$Models->name_en,['class'=>'form-control']) !!}
                     </div> 
        ...
        <input type="hidden" name="alreadyexist" class="department_id" value="{{ old('alreadyexist') }}">

   

         <div class="div_inpute col-sm-12 col-sm-12 col-md-12 col-lg-12">
          @foreach($Models->otherDataForModels_R()->get() as $data)
         <div>
          <div class="col-sm-1 col-sm-1 col-md-1 col-lg-1">
            ...
            @endforeach
              ...
              <a href="#" class="add_inpute btn btn-info" style="background-color:orange;color: #fff">add</i> </a>
                <div class="clearfix"> </div>
                <br>
         </div>
        ...
        <div class="form-group ">
            ...
            {!! Form::select('is_public', [null => trans('admin.is_public')] + ['yes' => trans('admin.yes'),'no'=>trans('admin.no')], null, ['class' => 'form-control']) !!}
        </div>
...
<button type="button" class="btn Orange" id="save_from_data1"><i class="fa fa-save"></i>      <i class="fa fa-spin fa-spinner loading-save-c hidden"></i>   {{trans('admin.save_continue')}}</button>
