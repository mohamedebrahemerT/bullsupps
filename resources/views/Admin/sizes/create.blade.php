 ...
 @push('js')
            <script type="text/javascript">
               ...
               $(document).on('click','.DELET_PRODUCT',function(){
                ...
                $.ajax({
              url:'{{ url('admin/DELET_PRODUCT/'.$sizes->id ) }}',
              dataType:'json',
              type:'post',
              data:{
                _token:'{{ csrf_token()  }}'
 ...
 @push('js')
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      @if($sizes->department_id) 
      

                    $.ajax({
       type:'GET',
       url:'{{ url('admin/dep_get_parent') }}',
       data:{country_id:'{{ $sizes->department_id }}',select:'{{ $sizes->city_id }}'},
       dataType: "html",
    success: function(data) {
        $('.city').html(data);
  ...
  {!! Form::open(['url'=>url('admin/sizes/'.$sizes->id),'files'=>true,'method'=>'put','id'=>'save_from_data' ]) !!}

 
<!---------------------------------------------------------------------->
     <div class="form-group col-md-2 col-lg-2 col-sm-2"> 
                 {{ Form::label('country_id', trans('admin.chiosemainprog'))  }}
{{ Form::select('country_id',App\Department::where('parent',null)->pluck('dep_name_ar','id'),$sizes->country_id,['class'=>'form-control country_id',"placeholder"=>"........"] )  }}
                     </div> 
...
<!----------------------------------------------------------------------->
        ...
        <input type="hidden" name="alreadyexist" class="department_id" value="{{ old('alreadyexist') }}">
         ...
         <div class="div_inpute col-sm-12 col-sm-12 col-md-12 col-lg-12">
          @foreach($sizes->otherDataForSize_R()->get() as $data)
         <div>
          <div class="col-sm-1 col-sm-1 col-md-1 col-lg-1">
