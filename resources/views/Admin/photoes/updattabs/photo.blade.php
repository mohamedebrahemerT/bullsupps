...
this.options.thumbnail.call(this,mock,"{{Storage::url($photoes->photo)}}" );
  ...
  $(document).ready(function(){
         $('#dropzone_file_uploads').dropzone({
            ...
            url:'{{ url("admin/HomeSlider/img/".$photoes->id) }}',
            paramName:'file',
            //uploadMultiple:true,
            maxFile:15,
            maxFilesize:3,
            acceptedFiles:'image/*',
            dictDefaultMessage:'<i class="fa fa-camera"></i> '+ '{{trans("admin.photoes")}}',
            dictRemoveFile:'{{trans("admin.dictRemoveFile")}}',
            removedfile:function(file)
            {
              //alert(file.id);
              $.ajax({
                dataType:'json',
                type:'post',
                url:'{{ url("admin/HomeSlider_delete_img")}}',
                data:{
 ...
 <hr>
          <h4 style="text-align: center;">{{trans('admin.photoes')}}</h4>
      ...
      <div class="dropzone" id="dropzone_file_uploads"> </div>
       ...
       <div class="div_inpute col-sm-12 col-sm-12 col-md-12 col-lg-12">
          @foreach($photoes->other_data_R_Add_color_for_slider()->get() as $data)
         <div>
          <div class="col-sm-2 col-sm-2 col-md-2 col-lg-2">
             {!! Form::label('input_key',trans('admin.input_key')) !!}
